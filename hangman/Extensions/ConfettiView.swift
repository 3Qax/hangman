...
private let kAnimationLayerKey = "com.towarek.animationLayer"
...
public final class ConfettiView: UIView {
    ...
    */
    public func emit(with contents: [(UIColor, UIImage)],
    ...
    private final class Layer: CAEmitterLayer {
        func configure(with contents: [(UIColor, UIImage)]) {
           emitterCells = contents.map { color, image in
                        ...
                        cell.contents = UIGraphicsImageRenderer(size: CGSize(width: 10, height: 10)).image {
                            _ in image.withTintColor(.white).draw(in:CGRect(origin:.zero, size: CGSize(width: 10, height: 10)))
                        }.cgImage
